# Forest Fire - Projeto Paralelo e Distribuído

## EC48A - Sistemas Distribuídos - C81
Repositório criado para o desenvolvimento do Projeto Final da disciplina de Sistemas Distribuídos do curso de Engenharia de Computação da UTFPR.

## Grupo 2
 - [<img src="https://i.imgur.com/6wtHdzd.png" width="30" height="30" style="vertical-align: middle;">](https://github.com/felipebataglini) Felipe de Oliveira Guimarães Bataglini 
 - [<img src="https://i.imgur.com/fA4JpJg.png" width="30" height="30" style="vertical-align: middle;">](target:_blankhttps://github.com/JoaoVBLaneiro) João Vitor Briganti Laneiro
 - [<img src="https://i.imgur.com/0ldubtT.png" width="30" height="30" style="vertical-align: middle;">](https://github.com/Noghiros) Stefano Calheiros Stringhini

## Dependências
- Python 3.8+
- numpy
- matplotlib (para plotar_graficos.py)
>- pip install numpy matplotlib

## Referências
- Drossel, B., & Schwabl, F. (1992). Self-organized critical forest-fire model. Physical Review Letters, 69(11), 1629–1632.

- Schenk, K., et al. (2001). The self-organized critical forest-fire model on large scales. arXiv.

- Karafyllidis, I., & Thanailakis, A. (1997). A model for predicting forest fire spreading using cellular automata (useful for applied CA-based fire models). 

- Ghosh, R. (2024). Fire Spread Modeling using Probabilistic Cellular Automata ([arXiv](https://arxiv.org/pdf/2403.08817)).

- Tudo feito com muita fé, força de vontade e leves ajudas do Google e de todas as ferramentas a nossa disposição.
